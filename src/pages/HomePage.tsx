...
const HomePage = () => {
  ...
  // Kontrollera om det faktiskt finns filter
  const hasFilters = filters && (filters.guests || filters.startDate || filters.endDate);

  const filteredProperties = hasFilters
    ? properties.filter((p) => {
        if (filters.guests && p.max_guests < filters.guests) {
          return false;
        }
        // TODO: lägg till datumfiltrering här
        return true;
      })
    : properties;
