...
export const CartProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const addItem: CartContextType['addItem'] = (item) => {
    setItems((prev) => {
      const idx = prev.findIndex(i => i.productId === item.productId && (i.variantId || null) === (item.variantId || null));
      if (idx >= 0) {
        const next = [...prev];
        next[idx] = { ...next[idx], quantity: next[idx].quantity + item.quantity };
        toast({ title: 'Added to cart', description: `${item.title} x${item.quantity}` });
        return next;
      }
  ...
  const updateQuantity: CartContextType['updateQuantity'] = (productId, variantId, quantity) => {
    setItems((prev) => prev.map(i => (i.productId === productId && (i.variantId || null) === (variantId || null)) ? { ...i, quantity } : i));
  };
  ...
  const value: CartContextType = { items, total, addItem, updateQuantity, removeItem, clear };
