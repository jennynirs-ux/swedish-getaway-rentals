    ...
    <Card className="w-full">
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3 p-4 bg-muted rounded-lg">
              ...
              <div className="space-y-1 text-sm">
                <div className="flex justify-between">
                  ...
                  <span>{Math.round(pricingCalculation.breakdown.accommodation / 100)} {currency}</span>
                </div>
                {pricingCalculation.breakdown.extraGuests > 0 && (
                  <div className="flex justify-between">
                    <span>Extra guests</span>
                    <span>{Math.round(pricingCalculation.breakdown.extraGuests / 100)} {currency}</span>
                  </div>
                )}
                {pricingCalculation.breakdown.cleaning > 0 && (
                  <div className="flex justify-between">
                    <span>Cleaning fee</span>
                    <span>{Math.round(pricingCalculation.breakdown.cleaning / 100)} {currency}</span>
                  </div>
                )}
                {pricingCalculation.breakdown.services > 0 && (
                  <div className="flex justify-between">
                    <span>Extra services</span>
                    <span>{Math.round(pricingCalculation.breakdown.services / 100)} {currency}</span>
                  </div>
                )}
                <div className="border-t pt-1 flex justify-between font-semibold">
                  <span>Total</span>
                  <span>{Math.round(pricingCalculation.total / 100)} {currency}</span>
                </div>
              </div>
            </div>
          )}
          ...
          >
            {loading ? 'Processing...' : `Book Now (${pricingCalculation.total > 0 ? `${Math.round(pricingCalculation.total / 100)} ${currency}` : 'Select dates'})`}
          </Button>
        </form>
      </CardContent>
    </Card>
