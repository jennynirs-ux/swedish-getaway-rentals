...
interface PropertyClientProps {
  initialLightProperty: Property | null;
  slug: string;
}

const NearbyPropertiesWrapper = memo(
  ({
    currentPropertyId,
    currentCoordinates,
  }: {
...
const PropertyClient = memo(({ initialLightProperty, slug }: PropertyClientProps) => {
  const router = useRouter();
  const [isGuideDialogOpen, setIsGuideDialogOpen] = useState(false);
  const [guideSectionId, setGuideSectionId] = useState<string | undefined>();
  const [lightProperty, setLightProperty] = useState<Property | null>(initialLightProperty);
  const [loading, setLoading] = useState(!initialLightProperty);
  ...
  // Fetch heavy property data
  const propertyHeavyQueryFn = useCallback(async () => {
    if (!lightProperty?.id) throw new Error('Property ID not available');
  ...
  const property = useMemo(() => {
    ...
    return {
      ...
      gallery_images: Array.isArray(heavyProperty?.gallery_images)
        ? heavyProperty.gallery_images
        : [],
      video_urls: Array.isArray(heavyProperty?.video_urls) ? heavyProperty.video_urls : [],
      gallery_metadata: Array.isArray(heavyProperty?.gallery_metadata)
        ? heavyProperty.gallery_metadata
        : [],
      video_metadata: Array.isArray(heavyProperty?.video_metadata)
        ? heavyProperty.video_metadata
        : [],
      amenities_data: Array.isArray(heavyProperty?.amenities_data)
        ? heavyProperty.amenities_data
        : [],
      guidebook_sections: Array.isArray(heavyProperty?.guidebook_sections)
        ? heavyProperty.guidebook_sections
        : [],
      special_highlights: Array.isArray(heavyProperty?.special_highlights)
        ? heavyProperty.special_highlights
        : [],
      featured_amenities: Array.isArray(heavyProperty?.featured_amenities)
        ? heavyProperty.featured_amenities
        : [],
...
PropertyClient.displayName = 'PropertyClient';
