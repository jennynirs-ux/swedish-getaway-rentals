-- Create sample properties for testing without requiring a specific host
-- This will populate the database with Villa Häcken and Lakehouse Getaway
...
WHERE NOT EXISTS (
  SELECT 1 FROM public.properties WHERE id = 'villa-hacken'::uuid
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM public.properties WHERE id = 'lakehouse-getaway'::uuid
);